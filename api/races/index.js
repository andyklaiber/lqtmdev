...
module.exports = async function (fastify, opts) {
    ...
    fastify.route({
        method: 'POST',
        url: 'results/:id/create-upload-url',
        preHandler: fastify.auth([fastify.verifyAdminSession]),
        handler: async function (request, reply) {
            const projection = {
                archived: 1,
            ...
            if (result) {
                ...
                const updatedData = {
                    resultsAuthToken: "test"
                }

                // let op = await this.mongo.db.collection('races').updateOne({ '_id': this.mongo.ObjectId(result._id) }, { $set: updateObject });
                return {resultsAuthToken: updatedData.resultsAuthToken, url: getUploadUrl(result.resultsAuthToken)}
            } else {
                return fastify.httpErrors.notFound();
            }
        }
